	...
	P = (t, e, n) =>
		e.has(t)
			...
			: e instanceof WeakSet
				? e.add(t)
				: e.set(t, n);
import { ba as Ut, aT as $e, K as C, t as O, F as N, M as De } from './CCKKAdte.js';
import { o as Zt } from './DCl4PXhq.js';
const B = [];
function jt(t, e = Ut) {
	let n = null;
	const a = new Set();
	function r(o) {
		if ($e(t, o) && ((t = o), n)) {
			const c = !B.length;
			for (const l of a) l[1](), B.push(l, t);
			if (c) {
...
var ce;
const x = ((ce = globalThis.__sveltekit_1iqfqam) == null ? void 0 : ce.base) ?? '';
var le;
const tn = ((le = globalThis.__sveltekit_1iqfqam) == null ? void 0 : le.assets) ?? x,
	en = '1772055428789',
...
async function Hn(t, e, n) {
	...
	document.URL !== location.href && (location.href = location.href),
		...
		E || ((E = T = Date.now()), history.replaceState({ ...history.state, [G]: E, [Z]: T }, ''));
	const a = V[E];
...
function _t({ url: t, params: e, branch: n, status: a, error: r, route: s, form: i }) {
	...
	(t.pathname = qe(t.pathname, o)), (t.search = t.search);
	const c = {
		type: 'loaded',
...
async function Kt({ loader: t, parent: e, url: n, params: a, route: r, server_data_node: s }) {
	var d, u, y;
	let i = null,
		o = !0;
	const c = {
...
function Te(t, e, n) {
	...
	const s = wt ? We(a, r.href, e) : He(a, e);
...
async function xe({ id: t, invalidating: e, url: n, params: a, route: r, preload: s }) {
	...
	if (p.some(Boolean)) {
		try {
			...
			const w = await K(g, { url: n, params: a, route: { id: t } });
			return J.has(s)
	...
	for (let g = 0; g < l.length; g += 1)
		if (l[g])
			try {
				...
				if (w instanceof qt) return { type: 'redirect', location: w.location };
				if (J.has(s))
					return ie({
						error: await K(w, { params: a, url: n, route: { id: r.id } }),
						url: n,
						params: a,
						route: r
					});
				...
				if (h != null && h.includes(w)) (b = w.status ?? b), (S = w.error);
					...
					if (await $.updated.check()) return await Ae(), await H(n);
					S = await K(w, { params: a, url: n, route: { id: r.id } });
				}
...
async function St({ status: t, error: e, url: n, route: a }) {
	...
	if (k.server_loads[0] === 0)
		try {
			...
			(n.origin !== bt || n.pathname !== location.pathname || Mt) && (await H(n));
		}
	try {
		const o = await Kt({
				loader: Ot,
		...
		if (o instanceof qt) return Ue(new URL(o.location, location.href), {}, 0);
		throw o;
	}
}
async function xn(t) {
	...
	try {
		const a = (async () => {
			let r =
				(await k.hooks.reroute({ url: new URL(t), fetch: async (s, i) => Te(s, i, t).promise })) ??
				t;
			if (typeof r == 'string') {
				const s = new URL(t);
...
async function Et(t, e) {
	if (t && !At(t, x, k.hash)) {
		...
		for (const r of Bt) {
			const s = r.exec(a);
			if (s) return { id: vt(t), invalidating: e, route: r, params: Ve(s), url: t };
		}
	}
}
function Pn(t) {
...
async function X({
	...
	const y = await Et(e, !1),
		f =
			t === 'enter'
				? Wt(v, y, e, t)
				: Pe({ url: e, type: t, delta: n == null ? void 0 : n.delta, intent: y });
	if (!f) {
		d(), F === c && (F = u);
		return;
	}
	...
	let h = y && (await xe(y));
	if (!h) {
		if (At(e, x, k.hash)) return await H(e);
		h = await Ce(
			e,
			{ id: null },
	...
	if (((e = (y == null ? void 0 : y.url) || e), F !== c))
		return f.reject(new Error('navigation aborted')), !1;
	if (h.type === 'redirect')
		if (o >= 20)
			h = await St({
				status: 500,
				error: await K(new Error('Redirect loop'), { url: e, params: {}, route: { id: null } }),
				url: e,
				route: { id: null }
			});
		else return await Ue(new URL(h.location, e).href, {}, o + 1, c), !1;
	else h.props.page.status >= 400 && (await $.updated.check()) && (await Ae(), await H(e));
	if (
		(In(),
		Vt(m),
		Re(p),
		...
		const g = s ? 0 : 1,
			w = { [G]: (E += g), [Z]: (T += g), [me]: i };
		(s ? history.replaceState : history.pushState).call(history, w, '', e), s || En(E, T);
	}
...
function Cn() {
	...
	const r = new IntersectionObserver(
		(o) => {
			for (const c of o) c.isIntersecting && (Pt(new URL(c.target.href)), r.unobserve(c.target));
		},
		{ threshold: 0 }
	);
	async function s(o, c) {
		const l = _e(o, j),
			d = l === e && c >= n;
		if (!l || d) return;
		const { url: u, external: y, download: f } = Ct(l, x, k.hash);
		if (y || f) return;
		const m = dt(l),
			p = u && vt(v.url) === vt(u);
		if (!(m.reload || p))
	...
	function i() {
		...
		for (const o of j.querySelectorAll('a')) {
			const { url: c, external: l, download: d } = Ct(o, x, k.hash);
			if (l || d) continue;
			const u = dt(o);
			u.reload ||
				(u.preload_code === q.viewport && r.observe(o), u.preload_code === q.eager && Pt(c));
		}
	}
	z.add(i), i();
}
	...
	(history.scrollRestoration = 'manual'),
		...
		j.addEventListener('click', async (n) => {
			...
			if (u !== void 0 && y) {
				...
				if (((W = !0), Vt(E), t(r), !c.replace_state)) return;
				W = !1;
			}
			n.preventDefault(),
				await new Promise((f) => {
		...
		addEventListener('popstate', async (n) => {
			...
			if ((a = n.state) != null && a[G]) {
				...
				if (c === T && (Se || l)) {
					i !== U.state && (U.state = i), t(o), (V[E] = Dt()), s && scrollTo(s.x, s.y), (E = r);
					return;
				}
				const u = r - E;
				await X({
		...
		addEventListener('hashchange', () => {
			W &&
				((W = !1), history.replaceState({ ...history.state, [G]: ++E, [Z]: T }, '', location.href));
		});
...
async function jn(
	...
	try {
		...
		(u = await St({
			status: pt(f),
			error: await K(f, { url: l, params: r, route: s }),
			url: l,
			route: s
		})),
			(t.textContent = ''),
...
export { Hn as a, Bn as l, U as p, $ as s };
